...
# =============================================================================
def get_mape_mse_n_scatter(x, y, trainY, yPred_train, testY, yPred, title):
    ...
    ax.scatter(trainY, yPred_train, color='green', edgecolors = 'slategray', linewidths=0.8, alpha=0.8, label='training chemicals', s=30, marker='v')
    ax.scatter(testY, yPred, color='gold', edgecolors = 'tan', linewidths=0.8 , alpha=1, label='testing chemicals', s = 30, marker='o')
    ...
    ax.text(0, 0.55, 'mse on test data: %.3f' %(total_mse), fontsize=11)
    ax.text(0, 0.51, 'mse on training data: %.3f' %(total_mse_train), fontsize=11)
    
    os.chdir('G:\\My Drive\\NUS\\Paper in Wang\'s lab\\papers\\2018 11 4 LCA_AI 2\\Results_best_scatter\\large range')
    my_legend = ax.legend(facecolor='black', loc='upper left')

    for text in my_legend.get_texts():
        text.set_color('White')
        
    fig.savefig(title + 'scatter', facecolor='w')
